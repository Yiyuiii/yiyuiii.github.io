...
`_data/translation_exemptions.yml` 是关闭的迁移清单，初始范围只来自本契约建立前已经存在的 11 篇单语文章，实际文件仅保留尚未配对的旧文。生产检查对清单外的 singleton 直接失败；一组译文完成后必须同步删除该 key 的豁免，残留豁免同样失败。不得用新增豁免规避新文章双语发布。
