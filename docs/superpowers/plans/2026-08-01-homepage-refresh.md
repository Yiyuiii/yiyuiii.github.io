...
首页内容流不在 `home.yml` 重复维护内容详情。三类来源通过稳定 ID、语言、标题、摘要、URL、`marker_date`、日期精度和可选图片归一化；标志日期从随笔初稿、GitHub 仓库 `created_at` 或论文权威首次公开记录解析，不随后续修订、本站整理、普通 GitHub push、Star/Fork 或错别字变化刷新。项目日期只表示仓库创建，不证明创建时已公开。排序为标志日期降序、同日 `id` 升序，首页显示最近 8 项。

“随机发现”（英文 “Random discovery”）在每次进入欢迎页时从最近 8 项之外随机抽取；使用浏览器安全随机源和拒绝采样，不记录访问、不读日期或历史、不使用 Cookie、localStorage、指纹或外部 API。中英文各自独立抽取；无 JavaScript 时仍有稳定可读的浏览起点。
...
4. **欢迎页与内容流**：人工可编辑文案、箭头/文本指引、统一 feed、随机发现、随笔索引迁址、品牌/导航/fallback/旧 tag 兼容。
