...
首页内容流不在 `home.yml` 重复维护内容详情。三类来源通过稳定 ID、语言、标题、摘要、URL、`first_public_date`、日期精度和可选图片归一化；日期从随笔初稿、GitHub `created_at` 或论文权威首次公开记录解析，不随后续修订、本站整理、普通 GitHub push、Star/Fork 或错别字变化刷新。排序为首次公开日期降序、同日 `id` 升序，首页显示最近 8 项。
