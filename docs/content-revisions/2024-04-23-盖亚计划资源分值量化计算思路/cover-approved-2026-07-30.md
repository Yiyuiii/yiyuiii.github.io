...
![《盖亚计划》终局版图与计分板](/assets/posts/202404232233/cover-bgg-5194524-square.webp)
...
总地来说，利爪在T5前吸魔都是稳赚不亏的，利爪的游戏目标就是尽可能吸魔用资源换产出滚雪球。扩张、爬贸易轨和起RL、AC都是可以考虑的选项。
...
上表的结论是，机器无论何时都不该在场上留TC，而RL的收益力压AC，因此3RL扩张机器成为资源最高的打法。

要塞的转换率高，但仅在能够即时出城且可以多出一城时赚，当回合的出城资源可以看作不少产出，需要谨慎计算。

**T1开局**，产出收益 AC+2m >= RL+2m = 2RL，如果资源允许，RL+4/5m是上限最高的开局
- 如果选择**RL开局**，因为机器人需要从m升级AC，代价太大，通常前期无法升级AC导致k收入缓慢，爬轨数量极少，只能走扩张/1盖亚发展路线。**RL开局**注意T4前尽量不要建3RL：c会溢出，同时在高科片的竞争中可能落后资源或动数。
- **知识AC**开局能凑齐4k产能，可自由选择扩张/盖亚/贸易的发展路线，但和其它种族相比开局资源转换率处于劣势，上限不高。和RL开局终局2轨相比，**知识AC**开局每多爬满一条轨，相当于种族天赋产生了4分收益，因此爬知识轨的路线附带10分左右的种族补偿。
