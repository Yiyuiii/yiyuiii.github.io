...
5. 并行加载这四个官方约 900px JPEG。全部在 10 秒内成功解码后才显示局部线索和四个完整候选；任一失败即结束本次尝试，不补候选、不续页、不自动重试；
6. 玩家作答后显示正确图片、作者、年代、CMA 来源页、CC0 与开放获取说明。

四张候选 `<img>` 全部解码后，脚本直接把已加载的答案 `Image` 放大裁切并绘制到线索 `<canvas>`；canvas 不再引用远程 URL，因此协议层只有四个媒体 URL，也不依赖缓存命中来避免第五次请求。跨域图片会使 canvas 进入浏览器的“污染”状态，本游戏不调用 `getImageData()`、`toDataURL()` 或其它读回／导出接口。开始下一题必须再次由玩家点击。

## 语言与题名

界面具有完整中英文文案，但 CMA 返回的馆藏题名没有可靠中文字段。中文页在揭晓时原样显示官方题名，不自动翻译、不伪造中文馆藏名；作者说明和年代也保留官方文本。候选作答阶段不显示题名，因此语言不会泄漏答案。
...
截至 2026-08-05：组件固定桩 Node 8 项、Python 6 项、Playwright 8 项通过；生产构建、站点契约、旧 URL 检查与全量 169 项 Playwright 回归通过。最终本地真实浏览器验收在 8 秒内以一次 CMA API 请求和四幅官方 JPEG 成题，桌面局部画布与候选图清晰，320 px 无横向溢出。组件只留在研究分支和 loopback 预览，尚未推送或发布。
