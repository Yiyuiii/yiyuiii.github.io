...
小于 `1536px` 的视口使用正文内原生折叠目录；从 `1536px` 起，目录进入 `13rem` 的左侧粘性栏，与 `72rem` 内容画布保持 `2rem` 间隔。阅读布局的边界同时包含正文和评论区：目录可以随评论继续滚动，但只在该布局内粘滞，会在页眉之后开始并在页脚之前收回。表格的横向溢出必须由表格自身承接，不得让页面产生横向滚动。正文可用 `.article-wide` 或 `.article-prose` 显式覆盖默认分层；独立图片段落在支持 `:has()` 的浏览器中自动进入宽内容画布，旧浏览器安全回退到正文宽度。
