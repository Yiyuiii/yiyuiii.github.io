...
组件输出一个 `<figure class="article-cover">`，其中图片最大高度为 `26rem`，保持原比例、水平居中且不裁切；窄屏使用可用自然宽度。图注使用统一的颜色、字号和段落节奏。中文图注保持直立，以小于正文的字号和次级颜色区分层级；英文图注可保留英文排版所需的斜体。依据与适用边界见 `docs/chinese-typography.md`。

`.article-cover__image` 与 `.post-content img` 是两套独立规则。不得给普通正文图片继承 `26rem` 限高。

文章阅读页使用两级宽度：标题、正文段落、列表、引用、脚注与普通代码块限制在 `50rem` 的可读宽度内；题图、独立正文图片、表格、非代码 `figure` 与 Mermaid 可使用最长 `72rem` 的宽内容画布。题图和正文图片都只放宽容器，不强制放大超过图片自然尺寸。确实需要展示超长源码或终端输出时，代码块可显式使用 `.article-wide`，并由自身横向滚动承接剩余宽度。

小于 `1536px` 的视口使用正文内原生折叠目录；从 `1536px` 起，目录进入 `13rem` 的左侧粘性栏，与 `72rem` 内容画布保持 `2rem` 间隔。阅读布局的边界同时包含正文和评论区：目录可以随评论继续滚动，但只在该布局内粘滞，会在页眉之后开始并在页脚之前收回。表格与代码块的横向溢出必须由各自容器承接，不得让页面产生横向滚动。正文可用 `.article-wide` 或 `.article-prose` 显式覆盖默认分层；独立图片段落在支持 `:has()` 的浏览器中自动进入宽内容画布，旧浏览器安全回退到正文宽度。
