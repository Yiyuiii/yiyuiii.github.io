...
首页用分类型“标志日期”作为 30 条内容的排序依据。三类日期语义不同，页面会明确说明；它们都不表示最近修订、最近 push、最近获得 Star、最近进入本站或最近由本站整理的时间。
...
| `writing:202608021600` | 2026-08-02 | 中英文 `revisions[0].date`，且等于文章 `date` |
| `writing:202608081000` | 2026-08-08 | 中英文 `revisions[0].date`，且等于文章 `date` |
| `writing:202608081030` | 2026-08-08 | 中英文 `revisions[0].date`，且等于文章 `date` |
| `writing:202608081100` | 2026-08-08 | 中英文 `revisions[0].date`，且等于文章 `date` |
| `writing:202608081130` | 2026-08-08 | 中英文 `revisions[0].date`，且等于文章 `date` |
...
按标志日期降序、同日稳定 ID 升序，当前“最近更新”前 8 条依次为：Minecraft 服务器随笔、小厨房随笔、四种颜色外套随笔、珍珠鸟随笔、《SETI》随笔、`codex-cc-tools`、`taco`、《大创造时代》随笔。推荐候选必须排除这 8 个身份；后续修订不会把旧随笔重新推入这里。
