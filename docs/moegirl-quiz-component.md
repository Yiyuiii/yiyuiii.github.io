...
固定的导言来源、名称遮蔽方式和网络隐私边界统一放在开始按钮前的披露段落中。出题后的线索框只显示“这是什么？ / What is this?”和本轮动态线索，不再重复机制说明。候选仍以角色相关信号做质量初筛，但标题可能是作品、书名等主题条目，因此用户界面不得把答案类型保证为角色。

当前只保留“匿名化导言 → 条目”这一种自然题型，不显示只有一个选项的题型面板。反向的“条目 → 四段导言”会同时铺开四段长文本，并使三个干扰项的节选来源与署名也进入界面；在移动端可读性、一次响应的可成题率和许可呈现上都明显更差。现有分类信号也不足以把人物、作品、书籍等拆成稳定的用户筛选项，强行配置会增加空批次而不是增加选择。以后若能从同一次响应形成真正短小、可核验且不增加请求的第二种线索形式，再按名画与 AniList 组件的多选设置模式扩展。
