...
- 中间级用几何递减的 `ΔE_OK` 目标；最终写入 CSS 的整数 RGB 必须反算后落在本级互不重叠的实际区间。除固定黑白端点外，两色最终 RGB 回算后的 OKLab 亮度中点必须位于 `0.52..0.68`，避免同题同时落入近黑或近白区。
- `2..20/25` 在上述中亮度区间内用 OKLab 构造后量化复核；`21..24/25` 从整数 RGB 邻域搜索并接受同一亮度约束；所有搜索有界并具有自动复算的确定性回退。`25/25` 的中性基值限制在 `112..152`，再只改红或蓝通道一个码值。
