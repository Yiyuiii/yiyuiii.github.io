...
- 默认布局的正式页面在正文后依次提供轻量反馈区与评论区：公开反馈进入 `.github/ISSUE_TEMPLATE/page-feedback.yml` 双语 GitHub Issue Form，私密反馈使用预填页面标题与绝对 URL 的电子邮件；评论使用仓库已启用的 GitHub Discussions、`Announcements` 公告分类与 Giscus，按严格 `pathname` 分离每个页面及中英文 URL。页面初次载入只渲染本地说明、按钮和 GitHub 直达链接，读者明确点击后才请求 `giscus.app` / GitHub；加载失败可重试，无 JavaScript 时保留直达链接。评论语言和明暗主题跟随当前页面，站点自身不为评论增加 Cookie 或存储；`giscus.json` 只允许正式域名与 loopback 本地预览嵌入。重定向兼容页与双语 404 排除评论，404 仍保留本地化反馈。2026-08-04，用户完成 Giscus GitHub App 的仓库级授权，Giscus 官方分类接口已返回仓库及 `Announcements` Node ID，CI artifact 上的中英文真实 iframe、登录入口与明暗主题同步均已验证。
