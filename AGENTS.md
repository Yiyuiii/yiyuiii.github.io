...
- GitHub Actions 工作流为 .github/workflows/deploy.yml；PR 与固定 `preview/replit` 测试分支的 push 只验证和上传 site-preview，master push 或在 master 上 `workflow_dispatch` 手动触发时，build 成功后 deploy。人工页面审阅使用 Replit 测试页面，不再向用户提供本机 localhost；但本地与 CI 的完整门禁仍统一由 `scripts/validate.py` 编排，CI 使用 `--browser`，在新 production 构建和静态检查后通过 `scripts/run_browser_tests.py` 启动随机 loopback 预览，以 Playwright 管理的 Chromium 和 4 worker 完成全套回归。Replit 的 Ruby 3.3 与原生扩展构建工具由 `replit.nix` 声明，锁定 Bundler 的运行入口为 `scripts/replit_preview.sh`；修改 Nix 环境后必须让 Replit Reload Shell。Replit 的 GitHub 导入不会随远端 push 自动拉取，发布页面也是快照，因此每轮必须在 CI 成功后显式拉取 `preview/replit` 并重新发布同一测试 URL；精确流程见 `docs/replit-preview-workflow.md`。预览服务器队列为 128，浏览器失败时保留 `browser-failure-artifacts`，不得绕过该门禁发布。
