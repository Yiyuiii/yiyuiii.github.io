...
- 2026-08-05 当前补充（优先于上一条中已经过时的单来源百科描述）：首项可见名称已改为“百科条目猜猜 / Encyclopedia entry quiz”，但稳定 ID 仍为 `moegirl-quiz`。中文页默认 `moegirl_zh`、可选 `wikipedia_zh`；英文页默认 `wikipedia_en`、可选明确标作中文线索的 `moegirl_zh`。每次明确点击最多一个官方 Action API GET，无预取、续页、图片、凭据或题目持久化；Wikipedia 只把唯一归入人物、地点、作品、组织、生物或事件的同类候选组成四选一，并在作答后链接准确 `oldid` 修订。当前实现仅在外部功能分支，固定桩和构建回归已通过；真实中英文 Wikipedia 质量闸门因本环境连接超时仍未评估，故不得发布。精确事实见 `docs/encyclopedia-quiz-component.md` 与 `docs/encyclopedia-quiz-live-feasibility-2026-08-05.json`。
