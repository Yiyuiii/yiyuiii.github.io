...
- GitHub Actions 工作流为 `.github/workflows/deploy.yml`：PR 只验证并上传 `site-preview`；固定 `preview/replit` 源码分支的 push 在完整门禁成功后，把已验证 `_site` 更新到只由 CI 写入的 `preview/replit-site`；只有 `master` push 或在 `master` 上手动触发时才执行正式 deploy。人工页面审阅使用 Replit，不再向用户提供本机 localhost；本地与 CI 的完整门禁仍统一由 `scripts/validate.py` 编排，CI 使用 `--browser`，以 Playwright 管理的 Chromium 和 4 worker 完成回归。Replit 只用 Python 托管生成分支中的静态快照，不承担 Ruby/Jekyll 构建；模板为 `scripts/replit_static_preview.replit` 与 `scripts/replit_static_preview.nix`。Replit App 为 `https://replit.com/@yiyuiii/yiyuiiigithubio`（ID `bf6868bf-44c5-4b08-a15f-0b95406a27b7`），固定公开审阅 URL 为 `https://yiyuiiigithubio--yiyuiii.replit.app`，源码哈希端点为同源 `/preview-source-sha.txt`；2026-08-08 已实测 Static 发布、`_site`、Run 静态命令及重点页面。GitHub 导入不会随远端 push 自动拉取，发布页也是快照；每轮 CI 成功后须显式 fetch，以 detached HEAD 核对 `origin/preview/replit-site`，再创建新的 Replit 本地 `replit-publish-<源码短哈希>-<序号>` 分支承接平台可能生成的 `Published your App` 提交，不合并、改写、删除或推送这些 Replit 本地记录。每次发布前必须进入 `Adjust settings` 显式确认 Static pages 与 `Public directory: _site`；Overview 即使仍显示 Static，`publicDir` 也可能为空并导致 `Could not find public directory`。发布成功后必须从固定 URL 核对 `/preview-source-sha.txt`。Starter 首个发布 App 免费但带标记并在 30 天后下线，可重新发布，不得为流程自动升级套餐。精确流程见 `docs/replit-preview-workflow.md`。预览服务器队列为 128，浏览器失败时保留 `browser-failure-artifacts`，不得绕过门禁发布。
...
- 11 篇旧文已全部完成双语配对；《SETI》以及 2026-08-08 从 Obsidian 外化的 Minecraft 最小运维闭环、小厨房最小系统、四色外套系统、珍珠鸟信任四组随笔也均有完整中英文版本，因此当前共有 16 组、32 篇随笔。`_data/translation_exemptions.yml` 现为保留架构的空闭集。新文章必须双语发布，不得新增豁免。
- 2026-08-08 已在 `content/evergreen-article-rewrite-20260808` 外部工作树完成 GitHub Pages、强化学习、云服务器和游泳四组中英文随笔重构，并同步完成无效资源声明清理、外链检查器加固、SCSS 八模块拆分与维护文档修正；随后将 `codex/obsidian-export-20260808` 工作树中的四组新随笔、12 项题图及索引派生资产、来源登记和首页条目合并进来，强化学习文章保留原常青教程并融合开放任务分布、元强化学习混淆、长时程和视觉预训练内容。加入 GitHub 测试分支与 Replit 审阅契约后的统一发布门禁已再次通过：392 项 Python、77 项 JavaScript、production Jekyll 构建、70 条旧 URL 和 169 项浏览器回归均成功；候选已推到固定 GitHub `preview/replit` 分支，CI 生成快照已同步并发布到固定 Replit 测试 URL，当前等待人工审阅，尚未合入或发布正式 master。范围、来源、交叉审阅修正和验证证据记录在 `docs/evergreen-article-rewrite-2026-08-08.md`；合并、继续修改或放弃工作树时必须同步更新。
...
- GitHub 测试分支与 Replit 审阅流程：docs/replit-preview-workflow.md
