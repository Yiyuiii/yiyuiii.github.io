...
- 2026-08-11 Python 本地检查、资源维护与 CI 依赖改由 `scripts/requirements-dev.in` 保存直接约束，`scripts/requirements-dev.txt` 使用 Python 3.12 与 `pip-tools==7.5.3` 固定全部版本和 PyPI 分发包哈希；Windows 通过条件依赖 `tzdata` 为 `zoneinfo` 提供 IANA 时区数据。CI 使用 `--require-hashes` 安装并运行 `pip check`。Pillow 保持 12.3.0，并受锁文件、两套图片策略和契约测试共同约束。GitHub 依赖图此前残留 12.0.0，告警刷新结果必须按当前 Dependabot、Dependency Graph 和 SBOM 复核；设计与证据见 `docs/dependency-security-baseline-2026-08-11.md`。
...
- 11 篇旧文已全部完成双语配对；《SETI》、2026-08-08 从 Obsidian 外化的四组随笔，以及《工业革命：匹兹堡》规则教学也均有完整中英文版本，因此当前共有 17 组、34 篇随笔。`_data/translation_exemptions.yml` 现为保留架构的空闭集。新文章必须双语发布，不得新增豁免。
