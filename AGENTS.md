...
- 《SETI》教学随笔应按实际开局与游玩顺序组织，文字简洁、条目清楚，避免无助于理解规则的比喻、长句和“不是……而是……”句式；“扩展理解”应优先解释不同来源的行动、组件和结算时点怎样组成规则链，外部知识只保留能直接帮助记忆或决策的浅层联系并控制字数，同时明确区分真实知识与游戏抽象；在完整讲清基础游戏后，另行覆盖《Space Agencies》、三种新增外星生物和官方促销内容，并只使用可核验来源的原装配图。
...
- 默认布局的正式页面在正文后依次提供轻量反馈区与评论区：公开反馈进入 `.github/ISSUE_TEMPLATE/page-feedback.yml` 双语 GitHub Issue Form，私密反馈使用预填页面标题与绝对 URL 的电子邮件；评论使用仓库已启用的 GitHub Discussions、`Announcements` 公告分类与 Giscus，按严格 `pathname` 分离每个页面及中英文 URL。页面初次载入只渲染本地说明、按钮和 GitHub 直达链接，读者明确点击后才请求 `giscus.app` / GitHub；说明与按钮披露当前空评论区首次加载约 `0.13 MB`，并注明实际用量随评论内容变化。加载失败可重试，无 JavaScript 时保留直达链接。评论语言和明暗主题跟随当前页面，站点自身不为评论增加 Cookie 或存储；`giscus.json` 只允许正式域名与 loopback 本地预览嵌入。重定向兼容页与双语 404 排除评论，404 仍保留本地化反馈。2026-08-04，用户完成 Giscus GitHub App 的仓库级授权，Giscus 官方分类接口已返回仓库及 `Announcements` Node ID，CI artifact 上的中英文真实 iframe、登录入口与明暗主题同步均已验证。
...
- 2026-08-03 至 2026-08-04：用户要求在每个正式页面底部增加基于 GitHub 的评论；本轮启用仓库 Discussions，并以 Giscus、严格路径映射、显式点击加载、双语与主题同步、来源限制和 GitHub 直达回退实现。用户随后完成 Giscus App 仓库级授权，官方接口和 CI artifact 真实 iframe 均已验证；精确边界以全站评论计划和当前测试为准。
