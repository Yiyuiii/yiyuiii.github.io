...
- 用户于 2026-08-23 明确授权新增网络延迟与波动检测器：节点必须来源可核验，并尽可能显示最细位置、网络类型、来源名称和测量特性；同时显示当前公网 IP、近似地区与网络归属，保留偶发高延迟和失败。没有可用来源的节点不做；浏览器无法证明的 CN2、AS9929、CMIN2 或其它“高速／低速”承载线路不得猜测。
...
- `/toys/` 与 `/en/toys/` 是双语“小玩意”索引，人工标题、说明、关键词与分组集中在 `_data/toys.yml`，由 `_includes/toy-index.liquid` 渲染为十二项单栏原生 `details` 与一项外部链接卡；页面一级标题仅供语义与无障碍使用，不在视觉上重复显示。页面输出十二项站内组件的完整 HTML 和 `noscript` 内容，另输出一项无需 JavaScript 的外部链接；站内项目运行时只由 `assets/js/toy-loader.js` 在首次展开、哈希直达或程序化打开时按固定同源白名单顺序加载；共享依赖只执行一次，失败资源可在本地化状态区重试。生产门禁限制加载器最多 `4 KiB`、折叠态全部同源 JavaScript 最多 `15 KiB`，并禁止项目运行时出现在初始脚本标签中；维护边界见 `docs/toy-loading.md`。当前十三项为萌娘百科猜猜、名画猜猜（克利夫兰艺术博物馆）、动画主角猜猜（AniList）、色差挑战、盲估十秒、反应时间、数字 Wordle、凑成 24、翻灯、随机密码、随机数字、网络延迟与波动检测器，以及 Apple 礼品码转相机可扫描卡片（外部）；前三项统一归入“知识问答／Knowledge quizzes”，后两项归入“实用工具／Utilities”，随机名字已移除。萌娘百科、名画与 AniList 三项只在用户明确开始后按披露边界请求官方公开服务；网络检测器也只在用户明确开始后请求用户启用的精确白名单端点；目录共 79 个节点、默认启用 10 个，启用节点以互不阻塞的独立循环并发采样，并在页面内逐个样本刷新公网 IP、近似归属、节点元数据、P50 与相邻波动紧凑列表、相应延迟曲线及诊断数据，不保存结果且不声称识别中间路由；DNSPod 与阿里公共 DNS 均按官方范围标为全球自动接入，不推定实际落点在中国大陆，精确边界见 `docs/toy-network-latency.md`；其余八项只在当前页面本地运行且不联网。外部数据游戏不得用固定小样本伪装题库；按局请求、许可、可达性、内容过滤与失败边界分别以组件文档和实时闸门为准。Apple 礼品码转相机可扫描卡片只输出指向 `https://diax7.github.io/redeem-apple-gift-cards-without-typing/` 的安全隔离链接；它把原本需要手动输入的文字礼品码转换成可供 Apple 设备摄像头扫描录入的卡片。本站不使用 `iframe`、不预连接或预取、不加载其字体与运行时，也不接触礼品码，精确边界见 `docs/toy-apple-gift-card.md`。色差、三款逻辑游戏与生成器不保存结果，盲估十秒与反应时间只允许使用 `yiyuiii.toy.ten-second.v1`、`yiyuiii.toy.reaction-time.v1` 两个本机键，各保留最近 100 次整数毫秒成绩并允许独立清空，不得保存时间戳、路径、设备或标识；正常状态的保存与隐私边界在操作区前集中说明一次，历史区只在浏览器拒绝存储时显示临时内存警告。数字 Wordle 默认使用四位互不重复的密码、八次机会，也可设置 `3..6` 位、允许重复及 `6/8/10/12` 次机会；用户界面以互不重叠的“完全命中”和“仅数字命中”表达反馈，允许重复时按多重集合计数，保留前导零且不预生成候选表；桌面历史表按内容收缩，不强制铺满游戏区；重新生成当前答案的按钮统一称“重置题目／Reset puzzle”。“显示答案／Show answer”只在进行中的一局可用，点击后以独立 `revealed` 状态结束该局，禁止继续提交，重置后恢复。凑成 24 枚举 `1..10` 的 715 个非降序四元组，完整可解题池为 566 组，其中 556 组存在全程正整数解、10 组必须使用分数；运算和显示始终使用约分后的精确分数，题池只提供“整数过程／需要分数／完整题池”。凑成 24 的“显示答案”在点击后即时求出三条可由现有按钮流程复现的精确等式，整数题池要求全程正整数、完整题池优先正整数解；查看后只锁住本次尝试，重置和换题仍可用，胜负后的原有撤销保持可用。翻灯枚举全部按键掩码并为每个可达局面保留最短代表解：3×3 有 512 个局面、4×4 有 4096 个，排除 0／1 步题后按精确最少 `2–3`、`4–5`、`6+` 步分层；棋盘支持网格语义、方向键、Enter／空格、逐步撤销与获胜后撤销。两款新游戏的数学与交互契约分别见 `docs/toy-make-24.md`、`docs/toy-lights-out.md`，独立 Python oracle 与浏览器回归不得删除。色差为 25 级三题结算积分制：每题答对 `+1`、答错 `-1`；默认用洗牌袋均衡轮换明暗、鲜淡、色相三种单轴题型及六段连续 OKLCH 色相，可选仅用于明暗题的中性灰，也可改为固定难度。答题后 16 个色块的实际 RGB、边框与不透明度保持不变，只以独立的绿色 `✓` 和红色 `×` 小角标标记答案与误选；不得再用整块降透明度或改色反馈。色差与三款逻辑游戏的设置都只在明确应用后生效并清空本局，不持久化。共同随机接口只接受 `crypto.getRandomValues()` 并用拒绝采样消除取模偏差，不得降级到 `Math.random`。
- 2026-08-24 网络延迟检测器以“开始测试”与运行状态作为顶部紧凑操作栏；其后增加默认折叠的“节点选择”，按自动接入、亚洲、大洋洲、欧洲、北美洲、南美洲和非洲分组。目录共 79 个来源明确且已通过真实 HTTPS 探针闸门的节点：3 个自动入口、13 个 HostHatch 固定机房、32 个 Vultr 固定都会区和 31 个 Akamai / Linode 固定地区测速主机；默认启用 10 个，按自动入口、香港、东京三家对照、新加坡、法兰克福和洛杉矶排列；Akamai / Linode 默认只保留东京，其余 30 个节点可选。用户可恢复默认、全选、清空或逐项勾选；只有启用节点出现在结果列表和图表并参与并发测试，运行时选择锁定，刷新后恢复默认，选择、样本与结果均不持久化。当前网络显示公网 IP／版本、运营商／网络、近似地区，并以常规字重的等宽数字显示“浏览器估计”RTT／下行；不显示容易被误读成物理制式的 `effectiveType` 原始 `4g` 档。P50 与波动图例使用和节点行一致的彩色圆点及数字阈值；测试开始并收到相应结果前只显示中性短横线，实际请求失败后才显示“无法获取”。访客归属按 IPIP.NET、IP.SB 顺序尝试，启用 Cloudflare 时其测速响应只作 IP 级最后补充，并显示本次实际来源。节点的位置、网络类型、来源、测量方式和特性并入各自“查看详情”；HostHatch 使用官方页面同源的约 0.2 KB `/api/health` 健康请求并按不透明响应计时，Akamai / Linode 使用官方云计算逐地区测速主机的不透明 HEAD，这些主机不是 CDN 就近边缘，Vultr 使用可读取状态的 HEAD。三家供应商身份说明只在对应节点详情出现；HostHatch 芝加哥以及 Vultr 大阪、明尼阿波利斯、俄亥俄和圣路易斯尚未通过当前闸门，因此未接入；完整解释见 `docs/toy-network-latency.md`。
...
- 2026-08-17 用户定向授权加入“Apple 礼品码转相机可扫描卡片”入口，随后选择将本站候选组件改为直接打开 diax7 上游页面的外部链接，以覆盖一般电脑没有私有字体的情况并避免本站托管字体；它不重启 M3/M4 或传统游戏扩充，维护说明见 `docs/toy-apple-gift-card.md`。
...
- CSP 不再使用泛化的 `https:` 来源：脚本仅允许同源、必要内联、jsDelivr、Giscus 与 Cloudflare Web Analytics；图片额外允许 CMA 官方图片 CDN；frame 仅允许 Giscus；connect 按实际功能逐项允许 Giscus、Cloudflare Web Analytics、AniList、CMA API、萌娘百科 API 与网络检测器的受检端点。外部服务维护探针都是显式 opt-in，不进入普通 CI；AniList 单请求入口为 `tests/tools/audit-acg-relation-quiz-live.mjs --run-live`，构建后外链维护入口为 `scripts/check_site.py --site _site --external-links`。
- 2026-09-03，`yiyuiii.top` 自定义域名与 Cloudflare Web Analytics 已由源提交 `232f4e2` 通过固定预览分支和生产工作流 `33711324865` 发布。构建产物携带根域 `CNAME`，canonical、hreflang、sitemap 与 Giscus backlink 使用 `https://yiyuiii.top`，Giscus 同时允许新旧正式来源；统计启动器只在 `yiyuiii.top` 及其子域注入用户提供的 beacon，本地预览不发送统计。Pages API 已显示 `cname=yiyuiii.top`、域名 `verified`、覆盖根域与 `www` 的证书 `approved`、强制 HTTPS 已开启；验证 TXT 公开可解析并须长期保留。Cloudflare 是权威 DNS 托管方，apex A/AAAA 与 `www` CNAME 直接指向 GitHub Pages，无需 Cloudflare 代理；根域返回 200，HTTP、`www` 与旧 `yiyuiii.github.io` 均按路径转到根域 HTTPS。构建契约限制每页至多一个 beacon，生产首页实测一个 beacon 且 RUM POST 返回 204，不产生 Cookie 或统计用本地存储；Cloudflare 面板聚合仍需等待上游处理，中国大陆上报完整率只作尽力而为。Giscus 在新域名能显示评论与登录界面，但既有 CSP 会拦截其父页 `default.css`，当前站内样式维持可用布局，是否放宽 `style-src` 留作独立维护项。精确发布证据、隐私边界和复核顺序见 `docs/cloudflare-web-analytics.md`。
...
- 11 篇旧文已全部完成双语配对；《SETI》、2026-08-08 从 Obsidian 外化的四组随笔、《工业革命：匹兹堡》规则教学、2026-08-12 新增的综合训练系统，以及 2026-08-23 新增的《荷塘月色》文学成品与通用构建流程两篇随笔也均有完整中英文版本，因此当前共有 20 组、40 篇随笔。`_data/translation_exemptions.yml` 现为保留架构的空闭集。新文章必须双语发布，不得新增豁免。
...
- 2026-08-23，《荷塘月色》全文影像随笔已在隔离候选工作树拆分为两组完整中英文文章：《把〈荷塘月色〉做成五分四十九秒的夜行》专注成片观看、文本分层、月光、声音、江南联想与回家结构；《从全文朗读到可验证影像：一套可撤回的构建流程》专注文本责任、真实朗读时间、状态化关键帧、声音分工、确定性装配、局部回退、机器检查与人工审美，并以 S023 波动、朗读接缝、平移黑边和封面重做为案例。两文互相链接并显著链接 Bilibili 公开视频 `BV1KQ8x6bEGk`，共用经核验的公开封面；资产来源、用途、转换与哈希见 `docs/article-assets/202608231837.yml`。该状态只表示网站文章候选，尚未授权 PR、合并或 Pages 发布。
...
- 2026-08-25，《从零器械到长期维护：适度肌肉量与综合体能的训练系统》已在仓库外隔离工作树形成主题与编排候选：正文以适度肌肉量、力量与力量体重比、爆发力、局部肌耐力、心肺能力和动作与专项技能六类结果为主轴，采用三次精简全身力量、两次渐进有氧和两次低疲劳爆发暴露；组数、RIR 与休息降为服务抗阻训练剂量的解释，24 个动作继续作为带进阶和退出条件的动作库。用户于 2026-08-26 明确授权按受保护流程发布；最终状态仍须以当前 Git refs、GitHub Actions 和正式 Pages 验收为准。删留、证据边界与验收要求见 `docs/cross-training-balanced-fitness-revision-2026-08-25.md`。
...
- `yiyuiii.top`、Cloudflare Web Analytics 与域名切换：docs/cloudflare-web-analytics.md
...
- 2026-08-25《综合训练系统》适度肌肉量与综合体能主题及编排修订底稿：docs/cross-training-balanced-fitness-revision-2026-08-25.md
...
- Apple 礼品码转相机可扫描卡片外部链接：docs/toy-apple-gift-card.md
