...
- 2026-09-03，`yiyuiii.top` 自定义域名与 Cloudflare Web Analytics 已形成未发布候选：构建产物携带根域 `CNAME`，canonical URL 改为 `https://yiyuiii.top`，Giscus 同时允许新旧正式来源；统计启动器只在 `yiyuiii.top` 及其子域注入用户提供的 beacon，本地预览和旧 GitHub 域名不发送统计。手动统计仅允许 `static.cloudflareinsights.com` 脚本与 `cloudflareinsights.com` 数据端点，页脚提供中英文隐私披露，阻断统计不得影响页面。Cloudflare 是权威 DNS 托管方，现有 apex A/AAAA 与 `www` CNAME 已直接指向 GitHub Pages 边缘，无需重建 DNS；GitHub Pages 尚未认领自定义域名，因此 HTTP 返回 GitHub 404，HTTPS 还会先遇到证书主机名不匹配。账户级 Pages 验证 TXT 当前不存在；必须先在 GitHub 个人设置验证 `yiyuiii.top` 并长期保留 GitHub 生成的 TXT，再允许发布，以关闭域名接管风险。发布、证书与最终域名验收顺序见 `docs/cloudflare-web-analytics.md`。
