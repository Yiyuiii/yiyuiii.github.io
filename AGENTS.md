...
- GitHub Actions 工作流为 .github/workflows/deploy.yml；PR 只验证和上传 site-preview；固定 `preview/replit` 源码分支的 push 在同一完整门禁成功后，还会把已验证 `_site` 更新到只由 CI 写入的 `preview/replit-site` 生成分支；master push 或在 master 上 `workflow_dispatch` 手动触发时，build 成功后 deploy。人工页面审阅使用 Replit 测试页面，不再向用户提供本机 localhost；但本地与 CI 的完整门禁仍统一由 `scripts/validate.py` 编排，CI 使用 `--browser`，在新 production 构建和静态检查后通过 `scripts/run_browser_tests.py` 启动随机 loopback 预览，以 Playwright 管理的 Chromium 和 4 worker 完成全套回归。Replit 只用 Python 托管生成分支中的静态快照，不承担 Ruby/Jekyll 构建，模板为 `scripts/replit_static_preview.replit` 与 `scripts/replit_static_preview.nix`。Replit App 为 `https://replit.com/@yiyuiii/yiyuiiigithubio`（ID `bf6868bf-44c5-4b08-a15f-0b95406a27b7`），固定公开审阅 URL 为 `https://yiyuiiigithubio--yiyuiii.replit.app`，源码哈希端点为同源 `/preview-source-sha.txt`；2026-08-08 已实测 Static 发布、`_site` 目录、Run 静态命令及重点页面。Replit 的 GitHub 导入不会随远端 push 自动拉取，发布页面也是快照，且 Static 发布会留下平台本地提交；因此每轮在 CI 成功后显式 fetch，并以 detached HEAD 指向 `origin/preview/replit-site`，保留但不合并、改写或推送 Replit 本地记录，再重新发布同一测试 URL。Starter 首个发布 App 免费但带标记并在 30 天后下线，可重新发布，不得为流程自动升级套餐。精确流程见 `docs/replit-preview-workflow.md`。预览服务器队列为 128，浏览器失败时保留 `browser-failure-artifacts`，不得绕过该门禁发布。
