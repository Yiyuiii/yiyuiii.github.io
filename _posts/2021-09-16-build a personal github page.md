...
revisions:
  ...
  - date: "2026-08-10"
    note: Standardized publishing, build, artifact, and deployment terminology; shortened repeated and high-load explanations
---
...
The durable lesson is to separate source, build environment, generated artifact, and public hosting. This model remains useful when themes and product interfaces change.
...
- A static-site generator compiles before deployment; Jekyll emits files, and Pages serves them afterward.
...
The official [site-creation guide](https://docs.github.com/en/pages/getting-started-with-github-pages/creating-a-github-pages-site) is the current authority for repository visibility, entry files, account plans, and setup screens. Recheck those product facts whenever configuring a site.
...
GitHub currently supports [publishing from a branch or with a custom GitHub Actions workflow](https://docs.github.com/en/pages/getting-started-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site). Both publish through Pages. They assign responsibility for the build differently.
...
This is an architecture example. The comments show the action versions checked on this article's revision date, while each `uses:` line pins the exact commit that was verified; GitHub's [secure-use guidance](https://docs.github.com/en/actions/reference/security/secure-use) identifies a full commit SHA as the immutable reference. Pin the full SHA, then let Dependabot or a deliberate maintenance pass propose reviewed updates. Keep the Ruby version in `.ruby-version` or set it explicitly in the workflow; the maintained [`ruby/setup-ruby` action](https://github.com/ruby/setup-ruby) reads the project version and can run `bundle install` from `Gemfile.lock` through `bundler-cache`. The build job has read-only repository and Pages access. The deployment job alone receives write and identity-token permissions and runs only for `main`. Workflow-level concurrency lets production commits finish the complete build-and-deploy sequence in order, preventing a slower old build from overwriting a newer site; a new run for the same pull request cancels its predecessor. `configure-pages` supplies the project-site base path to Jekyll. Add the project's own test commands between build and upload, and let pull requests execute the build without deploying. Protect the `github-pages` environment as a second branch guard. A different generator can replace the Ruby and build steps while preserving the artifact and deploy stages.
...
Maintain one production deployment path. Local scripts and continuous integration (CI) should produce the same artifact structure, and only that path should be authorized to publish it.

## Make Jekyll builds reproducible

### Know what each Ruby tool does

Jekyll is a Ruby gem. RubyGems distributes gems. A `Gemfile` declares the direct gems a project needs. Bundler resolves that graph, installs it, and records exact resolved versions in `Gemfile.lock`. `bundle exec` then runs a command inside that selected dependency environment, using the executable chosen by the project.
...
Branch publishing is a special case: GitHub's built-in Jekyll environment, supported plugins, and `github-pages` gem govern production compatibility. Use GitHub's current [Pages and Jekyll documentation](https://docs.github.com/en/pages/setting-up-a-github-pages-site-with-jekyll/about-github-pages-and-jekyll) to verify support for the local Jekyll version and third-party plugins.
...
Before pushing, run the production build and the project's checks:
...
GitHub also maintains a [local Jekyll testing guide](https://docs.github.com/en/pages/setting-up-a-github-pages-site-with-jekyll/testing-your-github-pages-site-locally-with-jekyll). A repository should document its required Ruby version and wrap any additional link, HTML, accessibility, or browser checks in one validation command. CI should call the same entry point so local and remote verification remain equivalent.
...
After a successful `jekyll build`, CI should verify these site behaviors at minimum:
...
Store a preview artifact for pull requests when visual review matters. Reviewers can then inspect the tested candidate output.
...
4. A clean local or CI build produces the exact artifact used by subsequent checks.
