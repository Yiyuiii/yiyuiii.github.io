...
excerpt: When a reinforcement learning experiment fails, inspect the task, hidden context, training distribution, episode boundary, and evaluation protocol before tuning the optimizer.
description: >-
...
revisions:
  ...
  - date: "2026-08-10"
    note: Standardized task-distribution, policy, evaluation, and replay terminology; reduced repeated qualifications and high-load sentences
---
...
The Markov property is a claim about a chosen state representation: conditional on that representation and the action, the distribution of the next state and reward no longer depends on earlier history. The latest sensor reading may fail this condition. If a camera frame hides velocity, a delayed actuator depends on earlier commands, or an opponent has private information, the observation is generally only a partial view of the state. The problem is then better described as a partially observable MDP (POMDP).
...
2. **Different tasks, similar trajectory quality:** can it separate factors that change optimal behavior?
...
The table provides an engineering starting heuristic. Primary entry points include the original work on [DQN](https://doi.org/10.1038/nature14236), [TD3](https://proceedings.mlr.press/v80/fujimoto18a.html), [SAC](https://proceedings.mlr.press/v80/haarnoja18b.html), and [PPO](https://arxiv.org/abs/1707.06347), plus an [offline RL tutorial and review](https://arxiv.org/abs/2005.01643) and a primary example of [doubly robust off-policy evaluation](https://proceedings.mlr.press/v48/jiang16.html). Read their assumptions and experimental protocols before transferring a method. The [Stable-Baselines3 algorithm guide](https://stable-baselines3.readthedocs.io/en/master/guide/rl_tips.html) can cross-check current action-space support and implementation details. Start with one mature baseline and a budget small enough to debug. Add recurrence, distributional value estimation, prioritized replay, auxiliary losses, or model learning after the simpler system establishes a trustworthy signal.
...
Prioritized experience replay (PER) samples transitions using a priority such as temporal-difference error and applies importance weights to reduce sampling bias. The original [PER paper](https://arxiv.org/abs/1511.05952) presents a replay scheme used throughout training. PER may improve learning efficiency, but noisy rewards, outliers, stale priorities, and reduced diversity can make a high error a poor proxy for usefulness. Use uniform replay as a control, log effective sample weights, and tune priority strength.
...
- [ElegantRL](https://github.com/AI4Finance-Foundation/ElegantRL) as another implementation source to inspect.
