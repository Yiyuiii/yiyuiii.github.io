(() => {
  ...
  const hongKongDate = `${part("year")}-${part("month")}-${part("day")}`;
  const candidateIdentity = candidates.map((item) => item.id).join("|");
  let hash = 2166136261;
  for (const stableId of `${hongKongDate}|${candidateIdentity}`) {
    hash ^= stableId.codePointAt(0);
  ...
  time.dateTime = selected.feed_date;
  time.textContent = selected.feed_date.replaceAll("-", rotation.dataset.dateFormat.includes("%Y.") ? "." : "-");
